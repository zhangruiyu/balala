...
class RestVerticle : CoroutineVerticle() {
    ...
    override suspend fun start() {
        ...
        log.info(commonRole)
        log.info(storeManagerRole)
        router.routeWithRegex(".*/common/.*").handler(JWTAuthHandler.create(authProvider).addAuthorities(commonRole))
        router.routeWithRegex(".*/storeNormal/.*").handler(JWTAuthHandler.create(authProvider).addAuthorities(storeNormalRole))
        router.routeWithRegex(".*/storeManager/.*").handler(JWTAuthHandler.create(authProvider).addAuthorities(storeManagerRole))
